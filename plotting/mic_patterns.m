...
%% MED OFF and ON separately (Figure S7a-b)
...
%% STIM OFF and ON separately (Figure S7c-d)

fbands = ["high_beta"];
regions = ["cortex", "STN"];
stims = ["off", "on"];
patterns_table = readtable(fullfile(folderpath_analysis, 'mic_topographies_cortex_STN_StimOffOn.csv'));
...
for stim_i = 1:length(stims)
    ...
    for region_i = 1:length(regions)
        ...
        for fband_i = 1:length(fbands)
            ...
            % interpolate patterns to mesh
            heatmap_fname = sprintf('MIC_%s_%s_Stim%s.nii', region, fband, stim);
            wjn_heatmap(heatmap_fname, coords, patterns - min(patterns), interp_mesh, [4, 4, 4])
            % (above saves file in current directory, so move to figures directory)
            movefile(heatmap_fname, sprintf('..\\figures\\%s', heatmap_fname));
            movefile("s"+heatmap_fname, sprintf('..\\figures\\s%s', heatmap_fname));
            ...
            camlight
            exportgraphics(gcf, fullfile(repo_path, 'figures', sprintf('MIC_%s_%s_Stim%s.png', region, fband, stim)), 'Resolution', 1000);
        end
    end
end
